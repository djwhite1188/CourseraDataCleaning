...
#  - CHECK! From the data set in step 4, creates a second, independent tidy data set with the average of each variable for each activity and each subject.
...
#combine test and training for X, Y, Subject
x <- rbind(x_test, x_train)
y <- rbind(y_test, y_train)
sub <- rbind(subject_test,subject_train)

#Create a key that will be used to link up the datasets
#!!!NOTE!!! - Assumption: all datasets are in the same order
a <- as.data.frame(seq(from=1, to=nrow(x), by = 1))
colnames(a) <- "key"

#Add column to y and subject
y_key <- cbind(y,a)
sub_key <- cbind(sub,a)
...
big1 <- merge(x_fin, y_fin, by = "key")
big_fin <- merge(big1, sub_key, by = "key")
